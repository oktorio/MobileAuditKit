...
_ASSESSMENT_TEMPLATE = _ENV.from_string(
    """<!doctype html><html><head><meta charset='utf-8'><title>MobileAuditKit Consolidated Assessment</title>
<style>body{font-family:system-ui;max-width:1240px;margin:2rem auto;padding:0 1rem;line-height:1.45}table{width:100%;border-collapse:collapse;margin:1rem 0}th,td{border:1px solid #ddd;padding:.55rem;text-align:left;vertical-align:top}.cards{display:flex;gap:.8rem;flex-wrap:wrap}.card,.finding,.notice{border:1px solid #ddd;border-radius:10px;padding:1rem;margin:.7rem 0}.card{min-width:130px}pre{overflow:auto;background:#f6f8fa;padding:.8rem}.PASS,.FAIL,.INCONCLUSIVE,.NOT_TESTED{font-weight:700}.muted{color:#666;font-size:.9em}</style></head><body>
<h1>MobileAuditKit Consolidated Assessment</h1>
<div class='notice'><b>Assessment ID:</b> {{ report.assessment_id }}<br><b>Profile:</b> {{ report.profile }} — {{ report.profile_description }}<br><b>Package:</b> {{ report.package or 'n/a' }} · <b>APK:</b> {{ report.apk or 'n/a' }}<br><b>APK SHA-256:</b> {{ report.metadata.apk_sha256 or 'n/a' }}<br><b>Flow:</b> {{ report.metadata.flow or 'n/a' }}<br><b>Tool version:</b> {{ report.tool_version }} · <b>Registry:</b> {{ report.metadata.registry_version or 'n/a' }}<br><b>Started:</b> {{ report.started_at }} · <b>Completed:</b> {{ report.completed_at }}</div>
{% if report.runtime_fingerprint %}<h2>Runtime fingerprint</h2><div class='notice'><b>ID:</b> {{ report.runtime_fingerprint.fingerprint_id }}<br><b>App:</b> {{ report.runtime_fingerprint.app_version_name or 'n/a' }} ({{ report.runtime_fingerprint.app_version_code or 'n/a' }})<br><b>Device:</b> {{ report.runtime_fingerprint.manufacturer or 'n/a' }} {{ report.runtime_fingerprint.model or '' }} · Android {{ report.runtime_fingerprint.android_version or 'n/a' }} · API {{ report.runtime_fingerprint.api_level or 'n/a' }} · {{ report.runtime_fingerprint.abi or 'n/a' }}<br><b>Frida:</b> {{ report.runtime_fingerprint.frida_version or 'n/a' }} · <b>Device ID hash:</b> {{ report.runtime_fingerprint.device_id_hash or 'n/a' }}{% if report.runtime_fingerprint.collection_errors %}<br><b>Collection notes:</b> {{ report.runtime_fingerprint.collection_errors|join(', ') }}{% endif %}</div>{% endif %}
{% if report.flows %}<h2>Flow markers</h2><table><thead><tr><th>Marker</th><th>Flow</th><th>Phase</th><th>Timestamp</th></tr></thead><tbody>{% for marker in report.flows %}<tr><td>{{ marker.marker_id }}</td><td>{{ marker.flow }}</td><td>{{ marker.phase }}</td><td>{{ marker.timestamp }}</td></tr>{% endfor %}</tbody></table>{% endif %}
<h2>Module coverage</h2><div class='cards'><div class='card'><b>PASS</b><br>{{ report.coverage.pass_count }}</div><div class='card'><b>FAIL</b><br>{{ report.coverage.fail_count }}</div><div class='card'><b>INCONCLUSIVE</b><br>{{ report.coverage.inconclusive_count }}</div><div class='card'><b>NOT TESTED</b><br>{{ report.coverage.not_tested_count }}</div><div class='card'><b>Execution coverage</b><br>{{ report.coverage.execution_coverage_percent }}%</div><div class='card'><b>Conclusive coverage</b><br>{{ report.coverage.conclusive_coverage_percent }}%</div></div><p class='muted'>{{ report.metadata.status_semantics }}</p>
<h2>Module evaluation</h2><table><thead><tr><th>Module</th><th>Engine</th><th>Status</th><th>Hook health</th><th>Threshold</th><th>Atomic tests</th><th>Observation</th><th>Evaluation</th></tr></thead><tbody>{% for m in report.modules %}<tr><td>{{ m.module }}</td><td>{{ m.engine }}</td><td class='{{ m.status }}'>{{ m.status }}</td><td>{% if m.hook_health %}{{ m.hook_health.state }}<br><small>{{ m.hook_health.hooks_installed }}/{{ m.hook_health.hooks_attempted }} hook groups · signal={{ m.hook_health.signal_received }}</small>{% else %}-{% endif %}</td><td>{{ m.fail_threshold }}</td><td>{{ m.test_ids|join(', ') or '-' }}</td><td>{{ m.observation }}</td><td>{{ m.evaluation }}{% if m.error %}<br><small>Error: {{ m.error }}</small>{% endif %}</td></tr>{% endfor %}</tbody></table>
<h2>Atomic test matrix</h2><p class='muted'>Atomic-test status is test-specific. A dynamic PASS is scoped to the exercised flow and sufficient hook/evidence coverage. It does not independently certify a MASVS control.</p><table><thead><tr><th>Test</th><th>Module</th><th>Status</th><th>Severity</th><th>MASVS</th><th>Observation</th><th>Evaluation</th><th>Evidence</th></tr></thead><tbody>{% for t in report.tests %}<tr><td>{{ t.test_id }}<br><small>{{ t.title }}</small></td><td>{{ t.module }}</td><td class='{{ t.status }}'>{{ t.status }}</td><td>{{ t.severity or '-' }}</td><td>{{ t.masvs|join(', ') or '-' }}</td><td>{{ t.observation }}</td><td>{{ t.evaluation }}</td><td>{{ t.evidence_ids|join(', ') or '-' }}</td></tr>{% endfor %}</tbody></table>
<h2>MASVS-linked test coverage</h2><p class='muted'>This matrix shows execution/conclusion of tests mapped to MASVS controls; it is not a compliance score.</p><table><thead><tr><th>MASVS control</th><th>Tests</th><th>PASS</th><th>FAIL</th><th>INCONCLUSIVE</th><th>NOT TESTED</th><th>Execution</th><th>Conclusive</th></tr></thead><tbody>{% for c in report.masvs_coverage %}<tr><td>{{ c.control_id }}</td><td>{{ c.total_tests }}</td><td>{{ c.pass_count }}</td><td>{{ c.fail_count }}</td><td>{{ c.inconclusive_count }}</td><td>{{ c.not_tested_count }}</td><td>{{ c.execution_coverage_percent }}%</td><td>{{ c.conclusive_coverage_percent }}%</td></tr>{% endfor %}</tbody></table>
<h2>Findings ({{ report.findings|length }})</h2>{% for f in report.findings %}<div class='finding'><h3>{{ f.finding_id }} — {{ f.title }}</h3><p><b>{{ f.severity }}</b> · {{ f.confidence }}{% if f.module %} · {{ f.module }}{% endif %}{% if f.test_id %} · {{ f.test_id }}{% endif %}</p><p>{{ f.description }}</p>{% if f.remediation %}<p><b>Remediation:</b> {{ f.remediation }}</p>{% endif %}<p><b>OWASP:</b> {{ f.owasp_mobile_top10|join(', ') }}<br><b>MASVS:</b> {{ f.masvs|join(', ') }}<br><b>MASWE:</b> {{ f.maswe|join(', ') }}<br><b>MASTG:</b> {{ f.mastg|join(', ') }}</p><p><b>Evidence:</b> {{ f.evidence_ids|join(', ') or '-' }}</p></div>{% else %}<p>No finding records were generated.</p>{% endfor %}
<h2>Evidence appendix ({{ report.evidence|length }})</h2><table><thead><tr><th>Evidence ID</th><th>Source</th><th>Type</th><th>Test</th><th>SHA-256</th><th>Redacted data</th></tr></thead><tbody>{% for e in report.evidence %}<tr><td>{{ e.evidence_id }}</td><td>{{ e.source }}</td><td>{{ e.evidence_type }}</td><td>{{ e.test_id or '-' }}</td><td><code>{{ e.sha256 }}</code></td><td><pre>{{ e.data_json }}</pre></td></tr>{% endfor %}</tbody></table></body></html>"""
)
...
def _payload(
    findings: Iterable[Finding], metadata: dict[str, Any] | None = None
) -> dict[str, Any]:
    items = [redact(f.model_dump(mode="json")) for f in findings]
    return {
        "tool": "MobileAuditKit",
        "metadata": redact(metadata or {}),
        "summary": dict(Counter(i["severity"] for i in items)),
        "findings": items,
    }
...
def write_html_report(
    ...
    for item in payload["findings"]:
        ...
        current["evidence_json"] = json.dumps(
            current.get("evidence", {}), indent=2, sort_keys=True
        )
        decorated.append(current)
    html = _FINDINGS_TEMPLATE.render(
...
def write_assessment_json(report: AssessmentReport, path: Path) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(_assessment_payload(report), indent=2, sort_keys=True),
        encoding="utf-8",
    )
    return path
...
def _sarif_rule(finding: Finding) -> dict[str, Any]:
    ...
    return {
        ...
        "help": {
            ...
            or "Review the MobileAuditKit finding and supporting evidence."
        },
        "properties": {
            "tags": [
                *finding.owasp_mobile_top10,
                *finding.masvs,
                *finding.maswe,
                *finding.mastg,
                *finding.cwe,
...
def _sarif_result(
    ...
    fingerprint = hashlib.sha256(
        f"{rule_id}|{finding.finding_id}|{default_location}".encode()
    ).hexdigest()
    return {
        "ruleId": rule_id,
        ...
        "locations": [
            {"physicalLocation": {"artifactLocation": {"uri": default_location}}}
        ],
...
def assessment_to_sarif(
    report: AssessmentReport,
    *,
    default_location: str = "AndroidManifest.xml",
) -> dict[str, Any]:
    findings = [
        finding for finding in report.findings if finding.severity != Severity.INFO
    ]
    ...
    results = [
        _sarif_result(
            finding,
            indexes[finding.test_id or finding.finding_id],
            default_location,
        )
        for finding in findings
    ]
    fingerprint_id = (
        report.runtime_fingerprint.fingerprint_id
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        ...
                        "informationUri": "https://github.com/oktorio/MobileAuditKit",
                        "rules": rules,
                    }
                },
                "automationDetails": {"id": report.assessment_id},
                "results": results,
                "properties": {
                    "profile": report.profile,
                    "flow": report.metadata.get("flow"),
                    "runtime_fingerprint_id": fingerprint_id,
                    "apk_sha256": report.metadata.get("apk_sha256"),
                    "registry_version": report.metadata.get("registry_version"),
                },
            }
        ],
    }
...
def write_assessment_sarif(
    ...
    *,
    ...
    path.write_text(
        json.dumps(
            assessment_to_sarif(report, default_location=default_location),
            indent=2,
            sort_keys=True,
        ),
        encoding="utf-8",
